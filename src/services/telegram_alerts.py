...
def _get_telegram_creds(channel: str = "default") -> tuple[Optional[str], Optional[str], str]:
    ...
    if ch in ("stock", "mirrorstock", "mirrorrastock"):
        token = os.getenv("MIRRORSTOCK_TELEGRAM_BOT_TOKEN") or os.getenv("MIRRORASTOCK_TELEGRAM_BOT_TOKEN")
        chat_id = os.getenv("MIRRORSTOCK_TELEGRAM_CHAT_ID") or os.getenv("MIRRORASTOCK_TELEGRAM_CHAT_ID")
...
def send_telegram_message(message: str, channel: str = "default") -> bool:
    """Send a message to Telegram using the selected channel credentials."""
    ...
    try:
        ...
        if response.status_code == 200:
            print(f"[TELEGRAM] Alert sent successfully → channel='{resolved}' chat_id={chat_id}")
            return True
        ...
        print(f"[ERROR] Telegram alert failed → channel='{resolved}': {e}")
        return False


def test_telegram(channel: str = "default") -> bool:
    """Quick sanity test you can call from anywhere."""
    return send_telegram_message("✅ Telegram test message: MirrorX backend is sending alerts.", channel=channel)
